...
interface ApproveRejectModalProps {
  ...
  onDualAction?: (reviewerId: number, reason: string) => void // New prop for dual action
  onReviewAndForward?: (reviewerId: number, reason: string) => void // New prop for review & forward
  loading: boolean
  success: boolean
  error: string | null
}
...
const ApproveRejectModal: React.FC<ApproveRejectModalProps> = ({
  ...
  onReject,
  onForward,
  onDualAction, // New prop
  onReviewAndForward, // New prop for review & forward
  loading,
  success,
  error,
}) => {
  ...
  const handleReview = () => {
    ...
    if (isReviewAndForwardChecked && reviewCount === 0) {
      // First mark as reviewed, then forward
      if (selectedReviewer) {
        // Call a new action for review and forward
        handleReviewAndForward(selectedReviewer, rejectionReason.trim())
      } else {
        // If no reviewer selected, just mark as reviewed
  ...
  // Replace the renderActionCheckboxes function with the following:

  const renderActionCheckboxes = () => {
    const reviewCount = leave?.review_count || 0;

    // 1. review_count === 0: Only Review & Forward and Reject
    if (reviewCount === 0) {
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <motion.div
            ...
            className={`flex items-center space-x-3 p-4 rounded-xl border-2 transition-all duration-300 ${isReviewAndForwardChecked
          ...
          >
            <Checkbox
              ...
              onCheckedChange={handleReviewAndForwardChange}
              disabled={loading}
          ...
          <motion.div
            whileHover={{ scale: 1.02 }}
            className={`flex items-center space-x-3 p-4 rounded-xl border-2 transition-all duration-300 ${isRejectChecked
    ...
    // 2. review_count === 2: Only Approve & Forward and Reject
    if (reviewCount === 2) {
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Approve & Forward */}
          <motion.div
            whileHover={{ scale: 1.02 }}
            className={`flex items-center space-x-3 p-4 rounded-xl border-2 transition-all duration-300 ${isApproveChecked && isForwardChecked
          ...
          >
            <Checkbox
              ...
              checked={isApproveChecked && isForwardChecked}
            ...
            <div className="flex items-center gap-2">
              <CheckCircle2
                className={`h-5 w-5 ${isApproveChecked && isForwardChecked ? "text-emerald-600" : "text-gray-400"}`}
              />
              <Forward
                className={`h-5 w-5 ${isApproveChecked && isForwardChecked ? "text-emerald-600" : "text-gray-400"}`}
              />
              <Label
                ...
                className={`font-semibold cursor-pointer ${isApproveChecked && isForwardChecked ? "text-emerald-700" : "text-gray-600"}`}
              >
                Approve & Forward
              </Label>
            </div>
    ...
    // 3. review_count > 2 and < 5: Only Approve and Reject
    if (reviewCount > 2 && reviewCount < 5) {
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <motion.div
            whileHover={{ scale: 1.02 }}
            className={`flex items-center space-x-3 p-4 rounded-xl border-2 transition-all duration-300 ${isApproveChecked
              ? "border-emerald-300 bg-emerald-50 shadow-emerald-100"
              : "border-gray-200 bg-white hover:border-emerald-200 hover:bg-emerald-50/50"
              }`}
          >
            ...
            <div className="flex items-center gap-2">
              <CheckCircle2
                className={`h-5 w-5 ${isApproveChecked ? "text-emerald-600" : "text-gray-400"}`}
              />
              <Label
                htmlFor="approve-action"
                className={`font-semibold cursor-pointer ${isApproveChecked ? "text-emerald-700" : "text-gray-600"}`}
              >
                Approve
              </Label>
            </div>
    ...
    // 4. review_count >= 5: Show approved success message only
    if (reviewCount >= 5) {
      return (
                ...
                <div className="p-6 space-y-6">
                  ...
                  {leave.organization && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: 0.4 }}
                      className="bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 rounded-2xl p-6 border border-green-100 shadow-lg hover:shadow-xl transition-all duration-300"
                    >
                      <div className="flex items-center mb-5 pb-3 border-b-2 border-gradient-to-r from-green-200 via-emerald-200 to-teal-200">
                        <div className="bg-gradient-to-br from-green-500 to-emerald-600 p-3 rounded-xl mr-3 shadow-md">
                          <Building className="text-white h-5 w-5" />
                        </div>
                        <div className="flex-1">
                          <h4 className="text-lg font-bold text-gray-800 uppercase tracking-wider">Organization Details</h4>
                          <p className="text-xs text-gray-500 font-medium">Company information</p>
                        </div>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {[
                          { label: "Organization", value: leave.organization.name, icon: Building },
                          { label: "Location", value: `${leave.organization.city}, ${leave.organization.country}`, icon: MapPin },
                          { label: "Address", value: leave.organization.address, icon: MapPin },
                          { label: "Contact", value: leave.organization.email, icon: Mail },
                        ].map((item, index) => (
                          <motion.div
                            key={item.label}
                            initial={{ opacity: 0, x: -20 }}
                          ...
                          >
                            <Label className="text-xs text-gray-500 font-bold uppercase tracking-wide mb-2 block">{item.label}</Label>
                            <div className="flex items-center gap-3 p-3 bg-white rounded-xl border border-gray-100 group-hover:border-green-200 group-hover:shadow-md transition-all duration-300">
