...
export default function CreateLeavePage() {
    ...
    // Use correct selectors for users and currentUser
    ...
    useEffect(() => {
        if (currentUser && currentUser.role === "employee") {
            setFormData(prev => ({ ...prev, employee_id: currentUser.id.toString() }));
        }
    }, [currentUser]);
    ...
    const handleReset = () => {
        setFormData({
            employee_id: currentUser?.role === "employee" ? currentUser.id.toString() : "",
            start_date: "",
            end_date: "",
            leave_type: "",
            reason: "",
    ...
    const validateForm = () => {
        if (!formData.employee_id || !formData.start_date || !formData.end_date || !formData.leave_type) {
                                            ...
                                            {reviewers.map((reviewer: any) => (
                                                <SelectItem key={reviewer.id} value={reviewer.id.toString()}>
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-sm text-gray-500 capitalize">{reviewer.role}</span>
                                                    </div>
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                            ...
                            <div className="flex gap-4 justify-center pt-6 border-t border-gray-200 dark:border-gray-700">
                                ...
                                <Link to="/dashboard/leaves">
                                    <Button
                                        type="button"
                                        variant="outline"
                                        className="px-8 py-3 font-semibold border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all duration-300 transform hover:scale-105 bg-transparent"
                                    >
                                        Cancel
                                    </Button>
                                </Link>
