...
interface TaskListProps {
  ...
  filters: { userName?: string } // Add filters prop
}
...
const TaskList: React.FC<TaskListProps> = ({ teamTasks, loading, onOpenReviewModal, filters }) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return (
          <Badge className="px-2 py-1 text-xs font-medium rounded-full bg-blue-500 text-white flex items-center gap-1">
  ...
  const filteredTeamTasks = useMemo(() => {
    ...
    // If no search term, return the username-filtered results
    if (!localSearch.trim()) {
      return filteredByUsername
    }

    // Apply search term filtering on top of username filtering
    return filteredByUsername
      .map((member) => {
        // Filter submissions that contain matching tasks
