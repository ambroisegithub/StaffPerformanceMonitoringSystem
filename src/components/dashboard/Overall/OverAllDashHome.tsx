  ...
  useEffect(() => {
    ...
    const fetchTaskStatusOverview = async () => {
      try {
        ...
        if (result && result.taskStatusOverview) {
          setTaskStatusData(result.taskStatusOverview);
        }
      } catch (error) {
      }
    };
      ...
      try {
        ...
        if (result) {
          if (result.employeePerformance) {
            setEmployeeData(result.employeePerformance);
          }
          if (result.summary) {
...
export default OverAllDashHomePage;