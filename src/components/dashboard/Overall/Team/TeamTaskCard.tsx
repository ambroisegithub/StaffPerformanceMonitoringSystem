...
import { CheckCircle, Clock, AlertCircle, Building, Briefcase, FileText, ChevronDown, Eye, User } from "lucide-react"
...
const TeamTaskCard: React.FC<TeamTaskCardProps> = ({ task, onSelect }) => {
  ...
  const getStatusBadge = () => {
    switch (task.review_status) {
      case "approved":
        return (
          <Badge className="px-2 py-1 text-xs font-medium rounded-full bg-green text-green border-green flex items-center gap-1">
      ...
      case "rejected":
        return (
          <Badge className="px-2 py-1 text-xs font-medium rounded-full bg-red text-white border-red flex items-center gap-1">
      ...
      default:
        return (
          <Badge className="px-2 py-1 text-xs font-medium rounded-full bg-blue text-white border-blue flex items-center gap-1">
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onSelect}
        className="w-full px-4 py-2 text-sm font-medium rounded-md flex items-center justify-center mb-3 bg-green text-white hover:bg-green-700"
      >
      ...
      <div className="text-xs text-gray-500 space-y-1.5 mb-3 mt-auto">
        <div className="flex items-center">
          ...
          <span className="truncate">{task.created_by?.name || "Unknown"}</span>
        </div>

        <div className="flex items-center">
          <Building className="h-3.5 w-3.5 mr-1.5 text-gray-400 flex-shrink-0" />
          <span className="truncate">{task.company || "N/A"}</span>
        </div>
