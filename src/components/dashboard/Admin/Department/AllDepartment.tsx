...
const ManageDepartment: React.FC = () => {
  ...
  const { departments, filteredDepartments, selectedDepartment, loading, error, success, successMessage, isEditing } =
  ...
  useEffect(() => {
    ...
    // Fetch companies
    axios
      .get<APIResponse<{ companies: ICompany[] }>>(`${import.meta.env.VITE_BASE_URL}/v1/${organizationId}/companies`, {
        headers: {
  ...
  useEffect(() => {
    if (Array.isArray(departments)) {
      const withCompany = departments.filter((dept) => dept.company !== null).length
  ...
  const getSortedDepartments = () => {
    ...
    const sortableDepartments = Array.isArray(filteredDepartments) ? [...filteredDepartments] : []
    ...
    <div className="px-4 py-8">
      <div className="flex flex-row justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 mb-6">Manage departments, assign to companies</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <Card className="h-28 flex flex-row">
          <CardContent className="flex flex-row justify-around w-full items-center">
            ...
            <p className="text-sm font-medium text-gray-600 mr-2">Total Departments</p>
            <h3 className="text-2xl font-bold text-gray-900">{departmentStats.total}</h3>
          </CardContent>
        </Card>
        ...
        <CardContent className="flex flex-row justify-around w-full items-center">
            ...
            <p className="text-sm font-medium text-gray-600">Departments with Company</p>
            <h3 className="text-2xl font-bold text-gray-900">{departmentStats.withCompany}</h3>
          </CardContent>
        </Card>
        ...
        <CardContent className="flex flex-row justify-around w-full items-center">
            ...
            <p className="text-sm font-medium text-gray-600">Departments without Company</p>
            <h3 className="text-2xl font-bold text-gray-900">{departmentStats.withoutCompany}</h3>
          </CardContent>
        </Card>
        ...
        <CardContent className="flex flex-row justify-around w-full items-center">
          ...
          <Building className="h-6 w-6 text-primary mx-auto mb-2" />
            ...
            {Object.keys(departmentStats.byCompany).length > 0 ? (
              Object.entries(departmentStats.byCompany).slice(0, 2).map(([company, count]) => (
      ...
      <Card className="w-full">
        <CardHeader className="pb-3">
          {error && (
            <div className="mb-4 p-3 border border-red text-red rounded-md flex gap-2">
              ...
              <button onClick={() => dispatch(clearDepartmentError())} className="ml-auto text-red hover:text-red">
                <X className="h-5 w-5" />
              </button>
            </div>
          )}
        ...
        <CardContent>
          ...
          {/* Pagination */}
          {Array.isArray(filteredDepartments) && filteredDepartments.length > itemsPerPage && (
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <Button
                      ...
                      className={currentPage === pageToShow ? "bg-green text-white" : "hover:bg-blue"}
                    >
                      {pageToShow}
                    </Button>
                  )
      ...
      <Dialog open={isViewUsersModalOpen} onOpenChange={setIsViewUsersModalOpen}>
        <DialogContent className="sm:max-w-lg">
          <div className="p-4">
            <h3 className="text-lg font-semibold mb-4">Users in {selectedDepartment?.name}</h3>
            ...
            <div className="mt-4 flex justify-end">
              <Button onClick={() => setIsViewUsersModalOpen(false)} className="bg-green hover:bg-blue text-white">
                Close
              </Button>
            </div>
          </div>
      ...
      <AlertDialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action will permanently delete this department. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDepartmentToDelete(null)}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmDelete} className="bg-red hover:bg-red-600 text-white">
              {loading ? (
                <>
                  <Loader className="mr-2 h-4 w-4 animate-spin" />
                  Deleting...
