                ...
                <Form className="space-y-6">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
                    ...
                    <p className="text-sm text-blue-700">
                      This position will be part of your organization's hierarchical structure:
                    </p>
                    <ul className="mt-2 text-sm text-blue-700 space-y-1">
                      <li>• <strong>Company:</strong> {selectedCompany?.name || 'Not selected'}</li>
                      <li>• <strong>Department:</strong> {
                        formValues.department_id 
                          ? filteredDepartments.find(d => d.id === Number(formValues.department_id))?.name || 'Not found'
                          : 'Not selected'
                      }</li>
                      <li>• <strong>Supervisory Level:</strong> {
                        formValues.supervisory_level_id
                          ? supervisoryLevels.find(s => s.id === Number(formValues.supervisory_level_id))?.level || 'Not found'
                          : 'Not selected'
                      }</li>
                      <li>• <strong>Reports To:</strong> {
                        formValues.direct_supervisor_id
                          ? availableSupervisors.find(s => s.id === Number(formValues.direct_supervisor_id))?.title || 'Not found'
                          : 'None (Top Level)'
                      }</li>
                    </ul>
                  </div>
